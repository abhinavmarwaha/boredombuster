...
const TMDBMovieBase = "https://www.themoviedb.org/movie/";
const TMDBTvBase = "https://www.themoviedb.org/tv/";
...
function addMovieList(){
    ...
    ++movieListCount;
    div.innerHTML  += '<div id="movieListHead' + movieListCount + '"> <input type="text" name="moviesList' + movieListCount + '" id="moviesList' + movieListCount + '" placeholder="TMDB list url" class="txtfield"> <button class="btn btn-small" id="moviesListNeg' + movieListCount + '" name="moviesList' + movieListCount + '">-</button> </div>';
    const elem = document.getElementById('moviesListNeg' + movieListCount);
    elem.addEventListener("click", function () {
        console.log("Deleting List...")
        removeMovieList(this.id);
    },false);
}
...
function addTvshowsList(){
    ...
    div.innerHTML += '<div id="tvshowsListHead'+tvshowsListCount+'"><input type="text" name="tvshowsList'+tvshowsListCount+'" id="tvshowsList'+tvshowsListCount+'" placeholder="TMDB list url"><button class="btn btn-small" id="tvshowsListNeg'+tvshowsListCount+'" name="tvshowsList'+tvshowsListCount+'">-</button></div>'
        ...
        tmdb.getConfiguration()
            .then(configs => {
                ...
                .then(data => data.results.forEach(element => {
                    ...
                    item.rating = element.vote_average;
                    item.summary = element.overview;
                    item.url = TMDBMovieBase + item.id;
                ...
                .then(() => {
                    ...
                    tmdb.getMovieVideos(items[selection].id).then((data)=>{
                        ...
                        displayVideoString += '<h4>'+Math.round(items[selection].rating*10)+'%</h4>';
    
                        displayVideoString += '<p class="summary-data">'+items[selection].summary+'</p>';
                        displayVideoString += '<a href= '+items[selection].url+'>TMDB</a><br><br><br><br>'
        ...
        .then(configs => {
            ...
            .then(() => {
                ...
                tmdb.getMovieVideos(items[selection].id).then((data)=>{
                    ...
                    displayVideoString += '<p class="summary-data">'+items[selection].summary+'</p>';
                    displayVideoString += '<a href= '+items[selection].url+'>IMDB</a>'
                    if(data.results[0].site=='YouTube'){
                        displayVideoString += '<iframe class="youtube" width="560" height="315" src="'+YoutubeBase +  data.results[0].key+'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
                    }
                    displayVideoString += '</div>';
...
function recommendAnime(){
    ...
    if(slots[rand]==0){
        jikanjs.loadTop('anime').then(
          ...
          .then(()=>{
            let selection = randNo(items.length);
            jikanjs.loadAnime(items[selection].id).then((data)=>{
                
                items[selection].summary = data.synopsis
                displayVideoString = "";
                ...
                displayVideoString += '<p class="summary-data">'+items[selection].summary+'</p>';
                displayVideoString += '<a href= '+ items[selection].mal_url+'>MAL</a><br><br><br><br>'
                displayVideoString += '<iframe class="youtube" width="560" height="315" src="'+data.trailer_url+'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
                displayVideoString += '</div>';
                items = [];
                displayVideoBlock.innerHTML = displayVideoString;
            })
...
function recommendManga(){
    ...
    if(slots[rand]==0){
        jikanjs.loadTop('manga').then(
          ...
          .then(()=>{
            let selection = randNo(items.length);
            jikanjs.loadManga(items[selection].id).then((data)=>{
                items[selection].summary = data.synopsis
                displayVideoString = "";
                displayVideoString += '<img src="';
                displayVideoString += items[selection].poster;
